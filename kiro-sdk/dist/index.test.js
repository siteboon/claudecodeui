...
function respondTo(mockProc, callIndex, result) {
    const written = mockProc.stdin.write.mock.calls[callIndex][0];
...
describe('query()', () => {
    ...
    it('resumes a session via session/load', async () => {
        ...
        await new Promise(r => setTimeout(r, 20));
        const loadReq = JSON.parse(mockProc.stdin.write.mock.calls[1][0]);
    ...
    it('interrupt() cancels the session', async () => {
        ...
        const cancelReq = JSON.parse(mockProc.stdin.write.mock.calls[cancelIdx][0]);
