...
describe('SessionRouter', () => {
  ...
  it('finish aggregates full text from assistant chunks', async () => {
    ...
    const msgs = [];
    for await (const msg of router.iterate('s1')) msgs.push(msg);
  ...
  it('iterate on unknown session returns immediately', async () => {
    const router = new SessionRouter();
    const msgs = [];
    for await (const msg of router.iterate('nonexistent')) msgs.push(msg);
  ...
  it('yields messages pushed after iterate starts', async () => {
    ...
    const done = (async () => {
      for await (const msg of router.iterate('s1')) collected.push(msg);
    })();
