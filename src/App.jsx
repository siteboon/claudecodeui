...
function AppContent() {
  ...
  const [autoExpandTools, setAutoExpandTools] = useLocalStorage(
    "autoExpandTools",
    false,
  );
  const [showRawParameters, setShowRawParameters] = useLocalStorage(
    "showRawParameters",
    false,
  );
  const [showThinking, setShowThinking] = useLocalStorage("showThinking", true);
  const [autoScrollToBottom, setAutoScrollToBottom] = useLocalStorage(
    "autoScrollToBottom",
    true,
  );
        ...
        if (selectedProject) {
          ...
          if (updatedSelectedProject) {
            ...
            if (selectedSession) {
              ...
              const updatedSelectedSession = allSessions.find(
                (s) => s.id === selectedSession.id,
              );
    ...
    const handleUpdateNow = async () => {
      ...
      try {
        ...
        if (response.ok) {
          ...
          setUpdateOutput(
            (prev) => prev + "Please restart the server to apply changes.\n",
          );
        } else {
  ...
  return (
    <div className="fixed inset-0 flex bg-background">
      ...
      <div
        className={`flex-1 flex flex-col min-w-0 ${isMobile && !isInputFocused ? "pb-mobile-nav" : ""}`}
      >
        <MainContent
          selectedProject={selectedProject}
          selectedSession={selectedSession}
...
// Detect proxy basename for orchestrator routing
// When accessed via /clients/{client_id}/proxy/, React Router needs this as basename
function getProxyBasename() {
  const path = window.location.pathname;
  const proxyMatch = path.match(/^(\/clients\/[^/]+\/proxy)/);
  return proxyMatch ? proxyMatch[1] : "";
}

// Root App component with router
function App() {
  const basename = getProxyBasename();
