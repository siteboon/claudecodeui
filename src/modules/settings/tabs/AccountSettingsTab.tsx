...
import type { PasswordFormState } from '@/modules/settings/tabs/accountSettings';
...
export default function AccountSettingsTab() {
  ...
  // Keeps the three password inputs together so validation always sees one consistent form snapshot.
  const [passwordForm, setPasswordForm] = useState<PasswordFormState>(initialPasswordFormState);
  // Surfaces validation and API failures beside the form without mutating the shared auth error state.
  const [error, setError] = useState<string | null>(null);
  // Prevents duplicate password-change requests while the first request is still in flight.
