...
const MessageComponent = memo(
  ({ message, prevMessage, createDiff, onFileOpen, showRawParameters, showThinking, selectedProject, provider, onEditMessage, onForkFromMessage }: MessageComponentProps) => {
    const { t } = useTranslation("chat");
    const isGrouped =
      prevMessage && prevMessage.type === message.type && (prevMessage.type === "assistant" || prevMessage.type === "user" || prevMessage.type === "tool" || prevMessage.type === "error");
    const messageRef = useRef<HTMLDivElement | null>(null);
    const userCopyContent = String(message.content || "");
    const formattedMessageContent = useMemo(() => {
      const content = formatUsageLimitText(String(message.content || ""));
      return provider === "codex" && message.type === "assistant" && !message.isThinking ? stripProposedPlanEnvelope(content) : content;
    }, [message.content, message.isThinking, message.type, provider]);
    const assistantCopyContent = message.isToolUse ? String(message.displayText || message.content || "") : formattedMessageContent;
    const isCommandOrFileEditToolResponse = Boolean(message.isToolUse && COPY_HIDDEN_TOOL_NAMES.has(String(message.toolName || "")));
    ...
    const shouldShowAssistantCopyControl = !isExporting && message.type === "assistant" && assistantCopyContent.trim().length > 0 && !isCommandOrFileEditToolResponse && !message.isThinking;
        ...
        {message.type === "user" ? (
          ...
          <div className="flex w-full items-end space-x-0 sm:w-auto sm:max-w-[85%] sm:space-x-3 md:max-w-md lg:max-w-lg xl:max-w-xl">
            <div className="flex min-w-0 flex-1 flex-col items-end gap-2 sm:flex-initial">
              {message.images && message.images.length > 0 && <ChatMessageImages images={message.images} projectId={selectedProject?.projectId} />}
              {message.files && message.files.length > 0 && <ChatMessageFiles files={message.files} />}
              {userCopyContent.trim().length > 0 || (!message.images?.length && !message.files?.length) ? (
                <div className="group max-w-full rounded-2xl rounded-br-md border border-border/60 bg-muted/60 px-3 py-2 text-foreground shadow-sm dark:bg-gray-800/60 sm:px-4">
                  <div dir="auto" className="break-words font-serif text-sm">
                    <Markdown breaks className="prose prose-sm max-w-none font-serif dark:prose-invert">
                      {message.content}
                    </Markdown>
                  </div>
            ...
            {!isGrouped && <div className="hidden h-8 w-8 flex-shrink-0 items-center justify-center rounded-full bg-blue-600 text-sm text-white sm:flex">U</div>}
          </div>
          ...
          <div className="w-full">
            {!isGrouped && (
              <div className="mb-2 flex items-center space-x-3">
                {message.type === "error" ? (
                  <div className="flex h-8 w-8 flex-shrink-0 items-center justify-center rounded-full bg-red-600 text-sm text-white">!</div>
                ) : message.type === "tool" ? (
                  <div className="flex h-8 w-8 flex-shrink-0 items-center justify-center rounded-full bg-gray-600 text-sm text-white dark:bg-gray-700">🔧</div>
                ) : (
            ...
            <div className="w-full">
              {message.isSubagentContainer ? (
                ...
                <>
                  <div className="flex flex-col">
                    <div className="flex flex-col">
                      <Markdown className="prose prose-sm max-w-none font-serif dark:prose-invert">{String(message.displayText || "")}</Markdown>
                    </div>
                  </div>
                  ...
                  {message.toolResult &&
                    ...
                    (message.toolResult.isError ? (
                      ...
                      <div id={`tool-result-${message.toolId}`} className="scroll-mt-4">
                        <ToolErrorDisplay label={t("messageTypes.error")} content={String(message.toolResult.content || "")} />
                      </div>
                    ) : (
                ...
                <Reasoning defaultOpen={isExporting}>
                  <ReasoningTrigger />
                  <ReasoningContent>
                    <Markdown className="prose prose-sm prose-gray max-w-none font-serif dark:prose-invert">{message.content}</Markdown>
                    {!isExporting && (
                ...
                <div dir="auto" className="text-sm text-gray-700 dark:text-gray-300">
                  ...
                  {(() => {
                    ...
                    if ((trimmedContent.startsWith("{") || trimmedContent.startsWith("[")) && (trimmedContent.endsWith("}") || trimmedContent.endsWith("]"))) {
                      try {
                          ...
                          <div className="my-2">
                            ...
                            <div className="overflow-hidden rounded-lg border border-border bg-muted">
                              <pre className="overflow-x-auto p-4">
                                <code className="block whitespace-pre font-mono text-sm text-foreground">{formatted}</code>
                              </pre>
                            </div>
                          </div>
                        );
                    ...
                    return message.type === "assistant" ? (
                      <StreamingMarkdown content={content} isStreaming={Boolean(message.isStreaming)} className="prose prose-sm prose-gray max-w-none font-serif dark:prose-invert" />
                    ) : (
              ...
              {Array.isArray(message.memoryCitations) && message.memoryCitations.length > 0 && <MemoryCitations citations={message.memoryCitations} />}

              {(shouldShowAssistantCopyControl || !isGrouped) && (
                <div className="mt-1 flex w-full items-center gap-2 text-[11px] text-gray-400 dark:text-gray-500">
                  {shouldShowAssistantCopyControl && <MessageCopyControl content={assistantCopyContent} messageType="assistant" />}
                  {shouldShowAssistantCopyControl && <MessageSpeakControl content={assistantCopyContent} />}
