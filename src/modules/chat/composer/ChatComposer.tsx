...
function useChatComposerController({
  ...
  const canQueueDraft =
    isLoading && Boolean(input.trim() || attachedFiles.length > 0);
  const submitHint = canQueueDraft
    ? hasQueuedDraft
      ? t("input.hintText.updateQueued", {
          defaultValue: "Enter to update queued message",
        })
...
function renderChatComposer({
  ...
  submitHint,
  submitAriaLabel,
}: ChatComposerProps & ReturnType<typeof useChatComposerController>) {
  return (
        ...
        <div className="relative mx-auto max-w-[54.25rem]">
            ...
            >
              {filteredFiles.map((file, index) => (
                <button
                  ...
                  key={file.path}
                  ref={
                    index === selectedFileIndex ? selectedFileRef : undefined
                  }
                  className={`cursor-pointer touch-manipulation border-b border-border/30 px-4 py-3 last:border-b-0 ${index === selectedFileIndex ? "bg-primary/8 text-primary" : "text-foreground hover:bg-accent/50"}`}
                  onMouseDown={(event) => {
          ...
          >
            ...
            <PromptInputFooter className="flex-wrap gap-y-1">
              ...
              <div
                className={`order-last hidden basis-full px-2 text-center text-xs leading-4 text-muted-foreground/50 transition-opacity duration-200 lg:block ${input.trim() && !canQueueDraft ? "opacity-0" : "opacity-100"}`}
              >
                {submitHint}
              </div>
            </PromptInputFooter>
