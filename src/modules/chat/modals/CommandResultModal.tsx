...
function formatRemainingCountdown(t: TFunction, resetTime?: string): string {
  ...
  if (diffMs <= 0) return t('cost.resetSoon', { defaultValue: 'Resets soon' });
  ...
  if (days > 0) {
    return t('cost.resetDaysHours', { days, hours, defaultValue: `Resets in ${days}d ${hours}h` });
  }
  if (hours > 0) {
    return t('cost.resetHoursMinutes', { hours, minutes, defaultValue: `Resets in ${hours}h ${minutes}m` });
  }
  return t('cost.resetMinutes', { minutes: Math.max(1, minutes), defaultValue: `Resets in ${Math.max(1, minutes)}m` });
}
...
function resolveBucketTitle(bucket: QuotaBucket, t: TFunction): string {
  ...
  if (isFiveHour) {
    return t('cost.fiveHourWindow', { defaultValue: '5-Hour Sliding Window' });
  }

  const isWeekly = bucket.window === 'weekly' || bucket.id.includes('weekly');
  if (isWeekly) {
    return t('cost.weeklyWindow', { defaultValue: 'Weekly Quota' });
  }

  const isCycle = bucket.window === 'cycle' || bucket.id.includes('calls');
  if (isCycle) {
    return t('cost.cycleWindow', { defaultValue: 'Cycle Request Limit' });
  }
...
function resolveBucketDescription(bucket: QuotaBucket, t: TFunction): string {
  ...
  if (isCycle) {
    ...
    if (zhMatch) {
      ...
      return t('cost.cycleCallsDesc', { total, remaining, defaultValue: `Total ${total} calls, ${remaining} remaining` });
    }
    return t('cost.cycleGeneralDesc', { defaultValue: desc || 'Cycle model & tool request quota' });
  }

  const isFiveHour = bucket.window === '5h' || bucket.id.includes('5h');
  if (isFiveHour && (bucket.description?.includes('token') || bucket.id.includes('tokens'))) {
    return t('cost.fiveHourRollingDesc', { defaultValue: '5-hour rolling token rate limit' });
  }
    ...
    <div className="overflow-hidden rounded-2xl border border-border/70 bg-background/75 transition-all duration-200">
      ...
      >
            ...
            <div className="flex items-center gap-2">
              ...
              {isCurrentGroup ? (
                <span className="shrink-0 rounded-md border border-primary/20 bg-primary/10 px-1.5 py-0.5 text-[10px] font-semibold text-primary">
                  {t('cost.currentGroup', { defaultValue: 'Active Session Models' })}
                </span>
              ) : (
                <span className="shrink-0 rounded-md border border-border/60 bg-muted/30 px-1.5 py-0.5 text-[10px] font-medium text-muted-foreground">
                  {t('cost.otherGroup', { defaultValue: 'Other Models' })}
                </span>
              )}
            </div>
            {groupDescription && (
        ...
        <div className="flex shrink-0 items-center gap-1.5 pl-3 text-xs text-muted-foreground">
          <span>{isExpanded ? t('cost.collapse', { defaultValue: 'Collapse' }) : t('cost.expand', { defaultValue: 'Expand' })}</span>
          {isExpanded ? (
              ...
              <div key={bucket.id} className="shadow-xs space-y-2 rounded-xl border border-border/60 bg-background/80 p-3">
                <div className="flex items-center justify-between gap-2">
                  ...
                  <span className={`shrink-0 font-mono text-xs font-bold ${tone.text}`}>
                    {t('cost.remainingPercent', { percent, defaultValue: `${percent}% remaining` })}
                  </span>
                </div>
...
function CostContent({ data, activeProvider }: { data: CostCommandData; activeProvider?: LLMProvider }) {
  const { t } = useTranslation('chat');
  const used = Number(data.tokenUsage?.used ?? 0);
  const total = Number(data.tokenUsage?.total ?? 0);
  const model = data.model || 'Unknown';
  const effectiveProvider = (data.provider && data.provider !== 'claude' ? data.provider : activeProvider || data.provider) || 'Unknown';
  const provider = getProviderLabel(effectiveProvider, effectiveProvider);
  const quotaProvider = resolveQuotaProvider(effectiveProvider);
  ...
  const fetchQuota = useCallback(async (isManualRefresh = false) => {
    ...
    try {
      ...
      setQuotaError(t('cost.fetchFailed', { defaultValue: 'Failed to load quota, please try again later' }));
    } finally {
  ...
  }, [fetchQuota, supportsQuota, data.quota]);
        ...
        <div className="space-y-3">
          <div className="flex items-center justify-between px-1">
            <h4 className="text-xs font-semibold uppercase tracking-[0.16em] text-muted-foreground">
              {t('cost.quotaTitle', { defaultValue: 'Account Quota & Rate Limits' })}
            </h4>
            <div className="flex items-center gap-2">
              {quotaData?.updatedAt && (
                <span className="text-[11px] text-muted-foreground/70">
                  {t('cost.updatedAt', {
                    ...
                    defaultValue: `Updated at ${new Date(quotaData.updatedAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}`,
                  })}
                </span>
              )}
              <button
                ...
                title={t('cost.refreshTitle', { defaultValue: 'Refresh latest quota' })}
              >
                <RotateCw className={`h-3 w-3 ${isRefreshing ? 'animate-spin text-primary' : ''}`} />
                <span>{t('cost.refresh', { defaultValue: 'Refresh' })}</span>
              </button>
            </div>
          </div>
            ...
            <div className="shadow-xs flex items-center gap-3 rounded-2xl border border-border/70 bg-background/75 p-4">
              ...
              <div>
                <p className="text-xs font-semibold text-foreground">
                  {t('cost.fetchingQuota', { defaultValue: 'Fetching latest quota...' })}
                </p>
                <p className="text-[11px] text-muted-foreground">
                  {t('cost.syncingWithProvider', { provider, defaultValue: `Syncing 5-hour and weekly limits from ${provider}` })}
                </p>
              </div>
            </div>
          )}
          ...
          {!loadingQuota && hasLoadedQuota && quotaGroups.length === 0 && (
            <div className="rounded-2xl border border-dashed border-border bg-background/60 px-4 py-6 text-center">
              <p className="text-xs font-semibold text-foreground">
                {quotaError || t('cost.noQuotaData', { defaultValue: 'No quota data available for this account' })}
              </p>
              {!quotaError && data.provider === 'codex' && (
                <p className="mt-1 text-[11px] text-muted-foreground">
                  {t('cost.apiKeyNotice', { defaultValue: 'API Key logins typically do not report 5-hour or weekly account limits' })}
                </p>
              )}
            </div>
          )}
          ...
          {!loadingQuota && quotaError && quotaGroups.length > 0 && (
            <div className="rounded-xl border border-amber-500/25 bg-amber-500/10 px-3 py-2 text-[11px] text-amber-700 dark:text-amber-300">
              {t('cost.refreshFailedNotice', { defaultValue: 'Refresh failed; showing last successfully retrieved quota data' })}
            </div>
          )}
    ...
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="flex h-[min(92dvh,48rem)] w-[calc(100vw-1rem)] max-w-5xl flex-col overflow-hidden rounded-3xl border-border/80 bg-popover/95 p-0 shadow-2xl backdrop-blur-xl sm:w-[min(94vw,64rem)]">
        ...
        <div className="settings-content-enter min-h-0 flex-1 overflow-hidden px-4 py-4 sm:px-6 sm:py-5">
          ...
          {payload?.kind === 'cost' && (
            <CostContent data={payload.data as CostCommandData} activeProvider={activeProvider} />
          )}
