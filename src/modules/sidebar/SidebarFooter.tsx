import { Settings, ArrowUpCircle, Bug, AlertTriangle, FileText } from 'lucide-react';
...
export default function SidebarFooter({
  ...
  return (
    <div className="flex-shrink-0" style={{ paddingBottom: 'env(safe-area-inset-bottom, 0)' }}>
      ...
      <div className="hidden px-2 py-1.5 md:block">
        <button
          className="mb-1 flex w-full items-center gap-2 rounded-lg px-2.5 py-1.5 text-muted-foreground transition-colors hover:bg-accent/60 hover:text-foreground"
          onClick={onShowDailyReport}
        >
          <FileText className="h-3.5 w-3.5" />
          <span className="text-sm">{t('actions.dailyReport')}</span>
        </button>
        <button
          className="flex w-full items-center gap-2 rounded-lg px-2.5 py-1.5 text-muted-foreground transition-colors hover:bg-accent/60 hover:text-foreground"
      ...
      <div className="px-3 pb-3 pt-2 md:hidden">
        <button
          className="mb-2 flex h-10 w-full items-center gap-3 rounded-xl bg-muted/40 px-3.5 transition-all hover:bg-muted/60 active:scale-[0.98]"
          onClick={onShowDailyReport}
        >
          <div className="flex h-7 w-7 items-center justify-center rounded-lg bg-background/80">
            <FileText className="h-4 w-4 text-muted-foreground" />
          </div>
          <span className="text-sm font-normal text-foreground">{t('actions.dailyReport')}</span>
        </button>
        <button
          className="flex h-10 w-full items-center gap-3 rounded-xl bg-muted/40 px-3.5 transition-all hover:bg-muted/60 active:scale-[0.98]"
          onClick={onShowSettings}
