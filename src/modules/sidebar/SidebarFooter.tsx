import { Settings, ArrowUpCircle, Bug, AlertTriangle, Github } from 'lucide-react';
...
export default function SidebarFooter({
  ...
  return (
    <div className="flex-shrink-0" style={{ paddingBottom: 'env(safe-area-inset-bottom, 0)' }}>
      ...
      {/* Footer actions as a single icon row: the labelled rows cost four lines of
          sidebar height for links that are followed rarely. Meaning is carried by
          title/aria-label, and the row is shared by desktop and mobile. */}
      <div className="nav-divider" />
      <div className="flex items-center justify-center gap-1 px-2 py-1.5">
        ...
        {!IS_PLATFORM && (
          <a
            ...
            className="group flex h-11 w-11 items-center justify-center rounded-lg transition-colors hover:bg-accent/80 md:h-8 md:w-8"
            aria-label={`CloudCLI v${currentVersion} – ${t('branding.openSource')}`}
            title={`CloudCLI v${currentVersion} – ${t('branding.openSource')}`}
          >
