    ...
    if (existingGroup) {
      ...
      if (
        ...
        (session.lastActivity &&
          session.lastActivity > existingGroup.latestActivity)
      ) {
...
function useSidebarContentController({
  isPWA,
  isMobile,
  isLoading,
  projects,
  ...
  const showConversationSearch =
    searchMode === "conversations" && searchFilter.trim().length >= 2;
  const hasSearchResults = Boolean(
    conversationResults &&
    (conversationResults.titleResults.length > 0 ||
      conversationResults.results.length > 0),
  );
  const groupedArchivedSessions =
    groupArchivedSessionsByProject(archivedSessions);
  const visibleArchivedItemsCount =
    archivedProjects.length + archivedSessions.length;
...
function renderSidebarContent({
  ...
  visibleArchivedItemsCount,
  isRenamingOnMobile,
}: SidebarContentProps & ReturnType<typeof useSidebarContentController>) {
  return (
    <div
          ...
          isSearching && !conversationResults ? (
            <div className="px-4 py-12 text-center md:py-8">
              ...
              {searchProgress && (
                <p className="mt-1 text-xs text-muted-foreground/60">
                  {t("search.projectsScanned", {
                    count: searchProgress.scannedProjects,
                  })}
            ...
            <div className="space-y-4 px-2" aria-live="polite">
              {conversationResults.titleResults.length > 0 && (
                <section
                  className="space-y-1"
                  aria-labelledby="session-title-results-heading"
                >
                            ...
                            {age && (
                              <>
                                ...
                                <time
                                  ...
                                  dateTime={session.lastActivity ?? undefined}
                                >
                                  {age}
                                </time>
                              </>
              ...
              {(conversationResults.results.length > 0 || isSearching) && (
                <section
                  className="space-y-3"
                  aria-labelledby="conversation-content-results-heading"
                >
                  <div className="flex items-center justify-between px-1 py-0.5">
                    <h3
                    ...
                    <div className="space-y-1.5 px-1">
                      <div className="flex items-center justify-end gap-1.5">
                        ...
                        <p className="text-[10px] text-muted-foreground/60">
                          {searchProgress.scannedProjects}/
                          {searchProgress.totalProjects}
                        </p>
                      </div>
                      <div className="h-0.5 overflow-hidden rounded-full bg-muted">
                        <div
                          ...
                          style={{
                            width: `${searchProgress.totalProjects > 0 ? Math.round((searchProgress.scannedProjects / searchProgress.totalProjects) * 100) : 0}%`,
                          }}
                        />
                      </div>
                    </div>
                        ...
                        >
                          ...
                          <div className="space-y-1 pl-4">
                            {session.matches.map((match) => (
                              <div key={match.messageUuid ?? `${match.timestamp}-${match.role}-${match.snippet}`} className="flex items-start gap-1">
              ...
              <p className="text-sm text-muted-foreground">
                {runningSessionsCount > 0
                  ? t(
                      ...
                      "No running sessions match this search.",
                    )
                  : t(
          ...
          isArchivedSessionsLoading ? (
            ...
            >
              <div className="flex items-center gap-2 px-1 py-2">
                ...
                <div>
                  ...
                  <p className="text-[11px] text-muted-foreground">
                    {t(
                      "archived.loadingDescription",
                      "Fetching hidden workspaces and sessions you can restore later.",
                    )}
                  </p>
                </div>
              </div>
              {[0, 1].map((skeleton) => (
                <div
                  key={skeleton}
                  className="animate-pulse rounded-xl border border-border/50 bg-card/40 p-3"
                >
            ...
            <div className="px-3 py-8 text-center">
              <div className="mx-auto max-w-[240px] rounded-2xl border border-dashed border-border/80 bg-muted/20 px-5 py-7">
                ...
                <p className="mt-1.5 text-xs leading-relaxed text-muted-foreground">
                  {archivedSessionsCount > 0
                    ? t(
                        "archived.tryDifferentSearch",
                        "Try a different search term.",
                      )
                    : t(
                        "archived.emptyDescription",
                        "Archived workspaces and sessions will appear here when you hide them from the active list.",
                      )}
                </p>
              </div>
            </div>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    ...
                    <p className="mt-1 text-[10px] leading-none text-muted-foreground">
                      {t(
                        "archived.restoreHint",
                        "Restore items whenever you need them",
                      )}
                    </p>
                  </div>
                </div>
                ...
                return (
                  ...
                  >
                    <div className="flex items-center gap-2.5 px-2.5 py-2.5">
                      ...
                      <button
                        className="flex h-7 flex-shrink-0 items-center gap-1.5 rounded-lg border border-emerald-600/15 bg-emerald-500/10 px-2 text-[10px] font-medium text-emerald-700 transition-colors hover:border-emerald-600/25 hover:bg-emerald-500/15 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-emerald-500/40 dark:text-emerald-300"
                        onClick={() =>
                          onRestoreArchivedProject(project.projectId)
                        }
                        title={t(
                          "archived.restoreProject",
                          "Restore workspace",
                        )}
...
export default function SidebarContent({
  ...
  const controller = useSidebarContentController({
    isPWA,
    isMobile,
    isLoading,
    projects,
