    ...
    return (
      <div className="h-full flex items-center justify-center">
        <div className="text-center text-gray-500 dark:text-gray-400">
          ...
          <h3 className="text-lg font-semibold mb-2">{t('empty.selectProject')}</h3>
          <p>{t('empty.chooseProject')}</p>
        </div>
      </div>
    );
  }
      ...
      <div className="flex-shrink-0 bg-gray-800 border-b border-gray-700 px-4 py-2">
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-2">
            ...
            {!selectedSession && (
              <span className="text-xs text-gray-400">{t('status.newSession')}</span>
            )}
            {!isInitialized && (
              <span className="text-xs text-yellow-400">{t('status.initializing')}</span>
            )}
            {isRestarting && (
              <span className="text-xs text-blue-400">{t('status.restarting')}</span>
            )}
          </div>
          <div className="flex items-center space-x-3">
            {isConnected && (
              <button
                ...
                title={t('actions.disconnectFromShell')}
              >
      ...
      <div className="flex-1 p-2 overflow-hidden relative">
        ...
        {!isInitialized && (
          <div className="absolute inset-0 flex items-center justify-center bg-gray-900 bg-opacity-90">
            <div className="text-white">{t('messages.loadingTerminal')}</div>
          </div>
        )}
          ...
          <div className="absolute inset-0 flex items-center justify-center bg-gray-900 bg-opacity-90 p-4">
            <div className="text-center max-w-sm w-full">
              ...
              >
                ...
                <span>{t('actions.continueInShell')}</span>
              </button>
              <p className="text-gray-400 text-sm mt-3 px-2">
                {selectedSession ? 
                  `${t('messages.resumeSession')} ${selectedSession.summary.slice(0, 50)}...` : 
                  t('messages.startNewSession')
                }
              </p>
            </div>
          </div>
          ...
          <div className="absolute inset-0 flex items-center justify-center bg-gray-900 bg-opacity-90 p-4">
            <div className="text-center max-w-sm w-full">
              <div className="flex items-center justify-center space-x-3 text-yellow-400">
                ...
                <span className="text-base font-medium">{t('actions.connectingToShell')}</span>
              </div>
              <p className="text-gray-400 text-sm mt-3 px-2">
                {t('messages.startingClaude')} {selectedProject.displayName}
              </p>
            </div>
          </div>
        )}
