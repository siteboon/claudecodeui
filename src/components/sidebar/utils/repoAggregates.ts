 ...
 * `__projectId` so the caller can route clicks to the correct worktree.
 */
export const getRepoSessions = (
  projects: Project[],
): (SessionWithProvider & { __projectId: string })[] => {
  const all: (SessionWithProvider & { __projectId: string })[] = [];
  for (const project of projects) {
    for (const session of getAllSessions(project)) {
      all.push({ ...session, __projectId: project.projectId });
    }
  }
