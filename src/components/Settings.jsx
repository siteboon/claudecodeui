...
function Settings({ isOpen, onClose, projects = [], initialTab = "agents" }) {
  ...
  const [newCursorDisallowedCommand, setNewCursorDisallowedCommand] =
    useState("");
  ...
  const deleteCodexMcpServer = async (serverId) => {
    try {
      const response = await authenticatedFetch(
        `/api/codex/mcp/cli/remove/${serverId}`,
        {
          method: "DELETE",
        },
      );
  ...
  useEffect(() => {
    localStorage.setItem(
      ...
      codeEditorShowMinimap.toString(),
    );
    window.dispatchEvent(new Event("codeEditorSettingsChanged"));
  }, [codeEditorShowMinimap]);
  ...
  useEffect(() => {
    localStorage.setItem(
      ...
      codeEditorLineNumbers.toString(),
    );
    window.dispatchEvent(new Event("codeEditorSettingsChanged"));
  }, [codeEditorLineNumbers]);
  ...
  const handleClaudeLogin = () => {
    ...
    setSelectedProject(
      projects?.[0] || { name: "default", fullPath: process.cwd() },
    );
                      ...
                      <div className="bg-gray-50 dark:bg-gray-900/50 border border-gray-200 dark:border-gray-700 rounded-lg p-4">
                        <div className="flex items-center justify-between">
                          <div>
                            ...
                            <div className="text-sm text-muted-foreground">
                              Display a minimap for easier navigation in diff
                              view
                            </div>
                          </div>
                          <button
                    ...
                    {selectedCategory === "mcp" &&
                      selectedAgent === "claude" && (
                        <McpServersContent
                          ...
                          onDelete={(serverId, scope) =>
                            handleMcpDelete(serverId, scope)
                          }
                          onTest={(serverId, scope) =>
                            handleMcpTest(serverId, scope)
                          }
                          onDiscoverTools={(serverId, scope) =>
                            handleMcpToolsDiscovery(serverId, scope)
                          }
                    ...
                    {selectedCategory === "mcp" &&
                      selectedAgent === "cursor" && (
                        <McpServersContent
                          ...
                          onEdit={(server) => {
                            /* TODO: Edit cursor MCP form */
                          }}
                          onDelete={(serverId) => {
                            /* TODO: Delete cursor MCP */
                          }}
                        />
                      )}
                    ...
                    {mcpFormData.importMode === "form" && editingMcpServer && (
                      <div className="bg-gray-50 dark:bg-gray-900/50 border border-gray-200 dark:border-gray-700 rounded-lg p-3">
                        ...
                        <p className="text-xs text-muted-foreground mt-2">
                          Scope cannot be changed when editing an existing
                          server
                        </p>
                      </div>
                    )}
                  ...
                  >
                    ...
                    <div className="flex justify-end gap-2 pt-4 border-t border-border">
                      ...
                      <Button
                        ...
                        disabled={
                          codexMcpLoading ||
                          !codexMcpFormData.name ||
                          !codexMcpFormData.config?.command
                        }
