...
export function useChatRealtimeHandlers({
  ...
  useEffect(() => {
    const handleEvent = (msg: ServerEvent) => {
      ...
      // NOTE: skip 'thinking' kind — each thinking delta arrives as a
      // separate message (one per word), which renders as individual blocks.
      // Thinking content is loaded correctly from JSONL after the run completes.
      const shouldPersist =
        msg.kind !== 'complete'
        && msg.kind !== 'status'
