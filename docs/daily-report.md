...
- Claude is the only AI summary provider in this MVP. It runs as an ephemeral Agent SDK query with built-in tools disabled, no MCP servers, no skills or settings sources, two structured-output protocol turns, and session persistence disabled.
- Reports are generated by Claude. If Claude is unavailable, times out, or returns invalid evidence references, generation fails clearly so a conversation list is never presented as an LLM summary.
- The summarizer uses the endpoint, credentials, and default model from the user's local Claude configuration; callers can explicitly request another configured Claude model.
...
- `POST /api/daily-reports/generate` accepts `date`, `timezone`, `locale`, optional `model`, and `summaryProvider: "claude"`.
