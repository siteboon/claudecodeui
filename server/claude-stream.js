...
function spawnClaudeProcess({ sessionId, cwd, model, permissionMode, mcpServers, additionalDirs }) {
  ...
  // Resolve CLI path eagerly — important on Windows where the SDK/spawn does
  // not always follow npm shell wrappers like cross-spawn does. Mirrors
  // claude-sdk.js after upstream e89d2da.
...
function sessionBelongsTo(session, userId) {
  ...
  // Coerce to string before comparing — different auth code paths may
