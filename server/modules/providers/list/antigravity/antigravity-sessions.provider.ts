...
export function cleanAntigravityMessageContent(
  ...
  // 1. Assistant messages: drop task-finish notifications, retries and server notices entirely.
  // This must run first, regardless of whether <SYSTEM_MESSAGE> is present.
  if (mode === 'assistant') {
  ...
  // 2. User messages: pure system notifications or wakeups must be completely stripped.
  if (mode === 'user') {
    let cleaned = text
