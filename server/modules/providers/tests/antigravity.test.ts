...
import { clearEnginePathCache } from '../list/antigravity/antigravity-engine-path.js';
import {
  getAntigravityDataRoot,
  getAntigravitySummariesDbPath,
} from '../list/antigravity/antigravity-data-root.js';
import {
...
test('resolveAntigravityModelArgs maps catalog selections onto agy arguments', () => {
  ...
  assert.deepEqual(
    resolveAntigravityModelArgs('gemini-3.1-pro-medium', undefined, proOption),
    { model: 'gemini-3.1-pro-high' },
  );
...
test('AntigravitySessionSynchronizer reads the summaries db from the overridden data root', async () => {
  ...
  try {
    summariesDb = new Database(getAntigravitySummariesDbPath());
    summariesDb.exec(`
...
test('getAntigravityDataRoot treats empty or whitespace CLOUDCLI_ANTIGRAVITY_DATA_DIR as unset', () => {
  const defaultPath = path.join(os.homedir(), '.gemini', 'antigravity-cli');

  const restoreEmpty = withEnvValue('CLOUDCLI_ANTIGRAVITY_DATA_DIR', '');
  try {
    assert.equal(getAntigravityDataRoot(), defaultPath);
  } finally {
    restoreEmpty();
  }

  const restoreSpaces = withEnvValue('CLOUDCLI_ANTIGRAVITY_DATA_DIR', '   ');
  try {
    assert.equal(getAntigravityDataRoot(), defaultPath);
  } finally {
    restoreSpaces();
