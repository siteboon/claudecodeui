  ...
  try {
    ...
    const wrappedCalls = [
      ...
      },
      {
        ...
        input: 'const r = await tools.exec_command({"cmd":"ls -la","workdir":"/workspace"}); text(r.output);',
        expectedToolName: 'Bash',
        expectedToolInput: JSON.stringify({ command: 'ls -la' }),
      },
      {
        callId: 'apply-patch-1',
        input: 'await tools.apply_patch("*** Begin Patch\\n*** Update File: src/test.ts\\n-old\\n+new\\n*** End Patch");',
        expectedToolName: 'Edit',
        expectedToolInput: JSON.stringify({ file_path: 'src/test.ts', old_string: 'old', new_string: 'new' }),
      },
