...
function handleChatConnection(ws) {
  ...
  ws.send = function (data) {
    try {
      ...
      // Capture sessionId for newly created sessions to enable PRETASK auto-run
      // Case 1: explicit session-created event
      if (parsed.type === "session-created" && parsed.sessionId) {
        lastCommandOptions = lastCommandOptions || {};
        lastCommandOptions.sessionId = parsed.sessionId;
      }

      // Case 2: claude-response messages that include session_id in the payload
      if (
        parsed.type === "claude-response" &&
        parsed.data &&
        parsed.data.session_id
      ) {
        // Only set if we don't already have a sessionId
        if (!lastCommandOptions || !lastCommandOptions.sessionId) {
          lastCommandOptions = lastCommandOptions || {};
          lastCommandOptions.sessionId = parsed.data.session_id;
        }
      }
      ...
      if (parsed.type === "claude-complete" && parsed.exitCode === 0) {
        ...
        if (lastCommandOptions && lastCommandOptions.sessionId) {
          ...
          setImmediate(async () => {
            try {
              await pretaskManager.checkAndExecuteNext(
                sessionId,
                projectPath,
                cwd,
                ws
      ...
      console.error(
        "Error parsing WebSocket message for pretask handling:",
        error
      );
    }
  ...
  ws.on("message", async (message) => {
    try {
      ...
      if (data.type === "init") {
        ...
        console.log(
          "📋 Session info:",
          hasSession ? `Resume session ${sessionId}` : "New session"
        );
