    ...
    if (isNaN(steps) || steps < 1) {
      return {
        ...
        data: {
          ...
          message:
            "Usage: /rewind [number] - Rewind conversation by N steps (default: 1)",
        },
      };
    }
...
router.post("/list", async (req, res) => {
  try {
    ...
    // Deduplicate by name: builtin > project > user > plugin (first seen wins).
    // allCommands is ordered builtin, project, user, plugin so iteration order
    // already reflects the desired precedence.
    ...
    uniqueCustomCommands.sort((a, b) => a.name.localeCompare(b.name));

    res.json({
      builtIn: builtInCommands,
      custom: uniqueCustomCommands,
      count: builtInCommands.length + uniqueCustomCommands.length,
    });
  } catch (error) {
...
router.post("/load", async (req, res) => {
  try {
    ...
    let realPath;
    try {
      realPath = await fs.realpath(commandPath);
    } catch {
    ...
    if (!checks.some(Boolean)) {
      return res.status(403).json({
        error: "Access denied",
        message:
          "Command must be inside an allowed project, user, or plugin commands directory",
      });
    }
