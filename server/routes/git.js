...
router.get("/diff", async (req, res) => {
  ...
  if (!project || !file) {
    return res
      .status(400)
      .json({ error: "Project name and file path are required" });
  }
  ...
  try {
    ...
    const isDeleted =
      statusOutput.trim().startsWith("D ") ||
      statusOutput.trim().startsWith(" D");
  ...
  try {
    ...
    if (isDeleted) {
      ...
      if (stats.isDirectory()) {
        ...
        return res
          .status(400)
          .json({ error: "Cannot show diff for directories" });
      }
...
router.post("/commit", async (req, res) => {
  ...
  if (!project || !message || !files || files.length === 0) {
    return res
      .status(400)
      .json({ error: "Project name, commit message, and files are required" });
  }
...
router.post("/checkout", async (req, res) => {
  ...
  if (!project || !branch) {
    return res
      .status(400)
      .json({ error: "Project name and branch are required" });
  }
...
router.post("/create-branch", async (req, res) => {
  ...
  if (!project || !branch) {
    return res
      .status(400)
      .json({ error: "Project name and branch name are required" });
  }
...
router.get("/commit-diff", async (req, res) => {
  ...
  if (!project || !commit) {
    return res
      .status(400)
      .json({ error: "Project name and commit hash are required" });
  }
...
router.post("/generate-commit-message", async (req, res) => {
  ...
  if (!project || !files || files.length === 0) {
    return res
      .status(400)
      .json({ error: "Project name and files are required" });
  }
 ...
 * @param {number} userId - User ID for model provider configuration
    ...
    if (provider === "claude") {
      await queryClaudeSDK(
        prompt,
        {
          ...
          model: "sonnet",
          userId: userId, // Pass user ID to enable model provider configuration
        },
        writer
      );
...
router.post("/publish", async (req, res) => {
  ...
  try {
    ...
    if (error.message.includes("rejected")) {
      ...
      errorMessage = "Remote not configured";
      details =
        "Remote repository not properly configured. Check your remote URL.";
    }
...
router.post("/discard", async (req, res) => {
  ...
  try {
    ...
    if (!statusOutput.trim()) {
      return res
        .status(400)
        .json({ error: "No changes to discard for this file" });
    }
...
router.post("/delete-untracked", async (req, res) => {
  ...
  try {
    ...
    if (!statusOutput.trim()) {
      return res
        .status(400)
        .json({ error: "File is not untracked or does not exist" });
    }
    ...
    if (status !== "??") {
      return res
        .status(400)
        .json({
          error: "File is not untracked. Use discard for tracked files.",
        });
    }
    ...
    if (stats.isDirectory()) {
      ...
      res.json({
        success: true,
        message: `Untracked directory ${file} deleted successfully`,
      });
    } else {
      await fs.unlink(filePath);
      res.json({
        success: true,
        message: `Untracked file ${file} deleted successfully`,
      });
    }
  } catch (error) {
